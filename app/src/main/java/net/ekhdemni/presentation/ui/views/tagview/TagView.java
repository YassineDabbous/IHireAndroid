...
import net.ekhdemni.presentation.ui.fragments.TagsFragment;
...
public class TagView extends RelativeLayout {
    ...
    private void drawTags() {
        ...
        for (Tag item : mTags) {
            ...
            TextView tagView2 = (TextView) tagLayout.findViewById(R.id.tv_tag_item_nbr);
            tagView.setText(tag.text);
            if(tag.number>0)
                tagView2.setText(tag.number+"");
            else
                tagView2.setVisibility(GONE);
    ...
    public void addTags(TagsFragment.TagClass[] tags) {
        if (tags == null) return;
        for (TagsFragment.TagClass item : tags) {
            Tag tag = new Tag(item.getName(), item.number);
            mTags.add(tag);
        }
        drawTags();
    }
